...
def run_benchmark(cases_path: Path, iterations: int) -> dict:
    ...
    for index, case in enumerate(cases):
        ...
        actual = replace_in_text(input_text, opts)
        if actual != expected:
            raise RuntimeError(
        ...
        for _ in range(iterations):
            replace_in_text(input_text, opts)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Python replace call-style benchmark")
    parser.add_argument("--cases", default="test/replace_cases.json")
